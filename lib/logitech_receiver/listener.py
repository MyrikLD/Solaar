...
class EventsListener(_threading.Thread):
    ...
    def __init__(self, receiver, notifications_callback):
        super(EventsListener, self).__init__(
            name=self.__class__.__name__ + ":" + receiver.path.split("/")[2]
        )
    ...
    def run(self):
        ...
        self.receiver.handle = _ThreadedHandle(
            self, self.receiver.path, self.receiver.handle
        )
