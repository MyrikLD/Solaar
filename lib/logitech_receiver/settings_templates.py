...
_HI_RES_SCROLL = (
    "hi-res-scroll",
    _("High Resolution Scrolling"),
    _("High-sensitivity mode for vertical scroll with the wheel."),
)
_HIRES_INV = (
    "hires-smooth-invert",
    _("High Resolution Wheel Invert"),
...
def check_feature_settings(device, already_known):
    ...
    def check_feature(name, feature_id, field_name=None):
        """
        :param name: user-visible setting name.
        :param feature_id: the numeric Feature ID for this setting.
        ...
        """
        if feature_id not in device.features:
            return
