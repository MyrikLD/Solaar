...
def _create_tree(model):
    ...
    battery_column.add_attribute(
        battery_cell_renderer, "icon-name", _COLUMN.STATUS_ICON
    )
...
def _find_selected_device_id():
    ...
    if item:
        return (
            ...
            _model.get_value(item, _COLUMN.NUMBER),
        )
...
def _device_row(receiver_path, device_number, device=None):
    ...
    receiver_row = _receiver_row(
        receiver_path, None if device is None else device.receiver
    )
...
def select(receiver_path, device_number=None):
    ...
    if item:
        ...
        _log.warning(
            "select(%s, %s) failed to find an item", receiver_path, device_number
        )
    ...
    if visible:
        ...
        def _details_items(device, read_all=False):
            ...
            if device.kind is None:
                ...
                yield (
                    _("Protocol"),
                    "HID++ %1.1f" % hid_version if hid_version else _("Unknown"),
                )
...
def _update_receiver_panel(receiver, panel, buttons, full=False):
    ...
    paired_text = (
        ...
        if devices_count == 0
            ...
            "%(count)s paired device.", "%(count)s paired devices.", devices_count
        )
        % {"count": devices_count}
    )
    ...
    if may_pair and devices_count >= receiver.max_devices:
        online_devices = tuple(
            n
            for n in range(1, receiver.max_devices)
            if n in receiver and receiver[n].online
        )
...
def _update_device_panel(device, panel, buttons, full=False):
    ...
    if is_online:
        ...
        if not_secure:
            ...
            panel._secure.set_tooltip_text(
                _(
                    "The wireless link between this device and its receiver is encrypted."
                )
            )
    ...
    if is_online:
        ...
        if light_level is None:
            ...
            panel._lux._icon.set_from_icon_name(
                _icons.lux(light_level), _INFO_ICON_SIZE
            )
...
def update(device, need_popup=False):
    ...
    if device.kind is None:
        ...
        assert (
            device.number is not None and device.number > 0
